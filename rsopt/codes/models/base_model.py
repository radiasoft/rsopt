...
class CommandModel(pydantic.BaseModel, typing.Generic[T]):
    commands: typing.List[T] = pydantic.Field(default_factory=list)
...
def generate_model(model_T, code_name):
    model_type = typing.Annotated[
        model_T, pydantic.Field(discriminator=DISCRIMINATOR_NAME)
    ]
    ...
    if _is_union(model_T):
        ...
        for m in typing.get_args(model_T):
            _field = pydantic.Field(default_factory=list)
            dynamic_fields[m.model_fields[DISCRIMINATOR_NAME].default] = (
                list[m],
                _field,
            )
    else:
        # Just a single Model
        _field = pydantic.Field(default_factory=list)
        dynamic_fields[model_T.model_fields[DISCRIMINATOR_NAME].default] = (
            list[model_T],
            _field,
        )

    dynamic_model = pydantic.create_model(
        model_name, __base__=base_model, **dynamic_fields
    )
