"""
This module contains methods to interface with various local
...
class LocaloptException(Exception):
    "Raised for any exception in localopt"
...
class LocalOptInterfacer(object):
    ...
    def __init__(self, user_specs, x0, f0, grad0=None):
        ...
        if isinstance(x_new, ErrorMsg):
            raise localoptException(x_new.x)
...
def run_local_nlopt(user_specs, comm_queue, x0, f0, child_can_read, parent_can_read):
    ...
    if return_val >= 1 and return_val <= 4:
        ...
        print("[localopt] The run started from " + str(x0) + " reached its maximum number "
              "of function evaluations: " + str(run_max_eval) + ". No point from "
              "this run will be ruled as a minimum! localopt may start a new run "
              "from some point in this run.")
        opt_flag = 0
    else:
        print("[localopt] NLopt returned with a negative return value, which indicates an error")
        opt_flag = 0
...
def run_local_scipy_opt(user_specs, comm_queue, x0, f0, child_can_read, parent_can_read):
    ...
    if res['status'] in user_specs['opt_return_codes']:
        ...
        print("[localopt] The SciPy localopt run started from " + str(x0) + " stopped"
              " without finding a local min.\nThe 'status' of the run is " + str(res['status']) +
              " and the message is: \"" + res['message'] +
              "\".\nNo point from this run will be ruled as a minimum! localopt may "
...
def run_local_dfols(user_specs, comm_queue, x0, f0, child_can_read, parent_can_read):
    ...
    assert 'bounds' not in user_specs.get('dfols_kwargs', {}), "localopt must set the bounds for DFO-LS"
    assert 'rhobeg' not in user_specs.get('dfols_kwargs', {}), "localopt must set rhobeg for DFO-LS"
    assert 'x0' not in user_specs.get('dfols_kwargs', {}), "localopt must set x0 for DFO-LS"
    ...
    if soln.flag == soln.EXIT_SUCCESS:
        ...
        print("[localopt] The DFO-LS run started from " + str(x0) + " stopped with an exit "
              "flag of " + str(soln.flag) + ". No point from this run will be "
              "ruled as a minimum! localopt may start a new run from some point "
              "in this run.")
        opt_flag = 0
...
def finish_queue(x_opt, opt_flag, comm_queue, parent_can_read, user_specs):
    ...
    if user_specs.get('print') and opt_flag:
        print('[localopt] Local optimum on the [0,1]^n domain', x_opt, flush=True)
