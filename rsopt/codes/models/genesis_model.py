...
from rsopt.codes.models import FormatterRegistry
...
class Io(BaseModel, extra='forbid'):
    ...
    magin: bool = Field(0, description="MAGINFILE: If set to a non-zero value the user is prompted to type in the file name containing a explicit description of the magnetic field. ")
    magout: bool = Field(0, description='Similar to MAGIN to write out the magnetic field lattice used for the simulation.')
...
class Undulator(BaseModel):
    ...
    def disperse_wcoefz(cls, data: dict) -> dict:
        if data.get('wcoefz'):
            ...
            data['wcoefz1'], data['wcoefz2'], data['wcoefz3'] = data.pop('wcoefz')
        return data
...
class Genesis(ElectronBeam, Focusing, Io, ParticleLoading, Mesh, Radiation,
    ...
    def model_dump(self, **kwargs):
        data = super().model_dump(**kwargs, warnings='none')
        formatted_data = {}
        for k, v in data.items():
            if isinstance(v, bool):
                ...
                v = FormatterRegistry.get_formatter(k)(v)
                formatted_data[k] = v
        return formatted_data


@FormatterRegistry.register('lout')
def _handle_lout(values):
